...
For the standardized experimental Relationship extension resource, use the
[`Relationship` base type](./relationship-base-type.mdx) instead of defining the base-type contract
manually.
...
prepaid billing (where a customer is charged _before_ the product is provisioned/fulfilled) to use
this pattern, allowing customers to interact with such resources through central experiences.

Unlike the envelope properties below, `BillingDataProperty` belongs in the resource-specific
properties model. Its `BillingData` schema is defined in ARM common types v6.
