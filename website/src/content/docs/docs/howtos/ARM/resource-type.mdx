...
## Adding Standard Resource Properties

An ARM resource can configure standard features through properties in its resource-specific
property bag or in the _ARM Envelope_. Unless noted otherwise, the following standard properties
...
- **Billing Data**: A standard property-bag model for prepaid-resource billing state and schedule.

### Billing Data

Billing Data provides a standard description of a resource's billing state, including a reference
to a product in the Marketplace Catalog. It is available with ARM common types v6 and is strongly
recommended for Resource Providers with prepaid billing (where a customer is charged _before_ the
product is provisioned or fulfilled), allowing customers to interact with such resources through
central experiences.

Unlike the envelope properties below, spread `BillingDataProperty` into the resource-specific
properties bag:
