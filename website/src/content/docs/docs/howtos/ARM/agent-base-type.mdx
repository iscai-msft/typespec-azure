...
property on create. `InputItem.type` selects a message, function call, function-call output, or
compaction item. Add the fields for that item type—for example, a message uses `content`, whose
value is an open `Record<unknown>`. Service-assigned fields such as `id`, `role`, and `status` are
read-only.
