...
`ConversationProperties` and `ResponseProperties` each require an `input: InputItem` property.
`InputItem.type` selects the message, function-call, function-call-output, or compaction shape.
Message `content` is `Record<unknown>` so the payload can carry structured content. The service
assigns `InputItem.id`, `InputItem.role`, and `InputItem.status`, which are read-only.
