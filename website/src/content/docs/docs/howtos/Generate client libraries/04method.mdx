...
```python
# The response body is no longer surfaced to the caller
...
```csharp
// The response body is no longer surfaced to the caller
public virtual Response Create(CancellationToken cancellationToken = default)
```
...
```python
# The caller now receives the raw response bytes
...
```csharp
// The caller now receives the raw response bytes
public virtual Response<BinaryData> Download(CancellationToken cancellationToken = default)
```
