...
The override operation's response type must also be compatible with the original operation's response type. An incompatible response type reports an `override-response-mismatch` error. To intentionally replace the response type, use [`replaceResponseWithVoid`](#replaceresponsewithvoid) or [`replaceResponseWithBytes`](#replaceresponsewithbytes), which report an informational `override-response-replacement` diagnostic that can be suppressed when the replacement is expected.
:::
...
Replace the method response type of an operation with `void`, discarding the client method return value while preserving the HTTP response metadata used for the wire protocol. Use this when the runtime response body should no longer be surfaced to callers of the generated client method, for example when a "delete" operation returns a body today but should return nothing to the SDK consumer:
...
Replace the method response type of an operation with the raw bytes of the HTTP response body, preserving the HTTP response metadata used for the wire protocol. Use this when callers of the generated client method should receive the unparsed response body instead of the modeled response type:
