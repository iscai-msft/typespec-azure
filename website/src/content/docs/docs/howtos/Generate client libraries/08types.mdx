...
:::caution
The default value's type must match the type of the property or parameter it is applied to. For example, passing a string default (`"10"`) to a numeric property, or a numeric default (`123`) to a string property, raises the `client-default-value-type-mismatch` warning. When `@alternateType` is also present, the default value is validated against the alternate type instead. If a mismatch is intentional you can suppress the diagnostic, but the mismatched default is still applied to the generated SDKs.
:::
