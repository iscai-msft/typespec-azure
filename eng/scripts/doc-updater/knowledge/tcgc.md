...
- `SdkBuiltInType` gained `wireType?: SdkBuiltInType` (`src/interfaces.ts`, populated in `addEncodeInfo` in `src/types.ts`). It is the built-in type a value is serialized as on the wire when `@encode` specifies a target encode-as type. `encode` still holds the encoding name; when no explicit encoding name is given, `encode` is set to `wireType.kind`.
- `addEncodeInfo` now allows `string` and `url` kinds (in addition to int and boolean) to be encoded as other built-in types.
- Documented in guideline.md "Built-in Types" bullet. Emitter type-graph metadata — no Spector spec needed (covered by existing `@encode` type tests).

## @clientDefaultValue type validation (Aug 2026)

- New diagnostic `client-default-value-type-mismatch` (`src/diagnostics/*.md`, emitted from `$clientDefaultValue.onTargetFinish` in `src/decorators.ts`). Warns when the value type passed to `@clientDefaultValue` does not match the target property/parameter type (string default on numeric property, etc.). When `@alternateType` is present, the value is validated against the alternate type instead.
- Documented as a "Value Type Validation" subsection in howto `08types.mdx` under the `@clientDefaultValue` section. Suppressible; suppressed mismatches still apply the default.

## New C# linter rules (Aug 2026)

- Two rules added to `src/linter.ts` (both `rules` and `csharpRules`): `csharp-model-suffix` (models should use recommended C# suffixes) and `csharp-use-standard-acronyms` (standard acronym casing). `reference/linter.md` already lists them (regenerated by the source PR). C#-only best-practice lint rules — no howto/guideline change needed.

## @operationGroup doc comment (Aug 2026)

- The `@operationGroup` deprecation note in `lib/decorators.tsp` was reworded (moved out of the `@deprecated` tag position into a plain "Deprecated: use `@client` instead." line). `reference/decorators.md` already reflects this (regenerated by the source PR). No further action.
