...
Rule explanations are authored beside implementations in
`packages/typespec-azure-resource-manager/src/rules/*.md` and connected through each rule's `docs`
field. `pnpm regen-docs` generates both `reference/linter.md` and the per-rule pages under
`website/src/content/docs/docs/libraries/azure-resource-manager/rules/`. The generated rule
directory is ignored by Git and is regenerated during the website build; never edit those pages
directly.

## Reference Documentation Generation

- `pnpm regen-docs` from `packages/typespec-azure-resource-manager/` generates the reference files
  (data-types.md, decorators.md, interfaces.md, index.mdx, linter.md) and per-rule pages.
- The package script passes `--rules-dir ../rules`, so per-rule pages are written beside
  `reference/`, not inside it.
...
- `Azure.ResourceManager.BaseTypes.Relationships.Relationship<Properties>` is an
  `ExtensionResource` template that applies
  `@azureBaseType(#{ baseType: BaseType.Relationship, version: "2026-04-01" })`.
- `RelationshipProperties<ProvisioningState = ResourceProvisioningState>` supplies the required
  read-only `baseTypes` and `provisioningState` properties plus writable `sourceId`,
  `sourceTenant`, `targetId`, and `targetTenant`.
- The `use-relationship-required-properties` rule requires Relationship resources to be extension
  resources with all required properties.
- The canonical sample is
  `packages/samples/specs/resource-manager/resource-types/relationship/main.tsp` and uses
  `Extension.Read`, `Extension.CreateOrReplaceAsync`, `Extension.CustomPatchAsync`,
  `Extension.DeleteWithoutOkAsync`, and `Extension.ListByTarget`.

## Billing Data

- `BillingData`, its schedule models, and related unions are available in ARM common types v6.
- `BillingDataProperty` is spread into the resource-specific properties bag, not the resource
  envelope. This is an exception to the usual placement of standard ARM property mix-ins.
- The canonical sample is
  `packages/samples/specs/resource-manager/resource-common-properties/billing-data/main.tsp`.
