...
Rule doc files under `website/src/content/docs/docs/libraries/azure-resource-manager/rules/` are **gitignored** (`.gitignore` ignores `website/src/content/docs/docs/libraries/*/rules/`) and generated at website build time from the tracked source rule docs at `packages/typespec-azure-resource-manager/src/rules/*.md`. Never hand-edit the website copies — they will not be committed and are overwritten. Edit the `src/rules/<rule>.md` file instead. They are not touched by `pnpm regen-docs`. The `reference/linter.md` file IS auto-generated by `regen-docs` and links to these rule doc files. The rule doc frontmatter `title` should match the **markdown file name** (not the registered rule name in `src/linter.ts`), because the doc URL is derived from the file name. The "Full name" code block inside the doc still shows the fully-qualified registered rule name (which may include an `arm-` prefix).
...
- Agent `@azureBaseType` version bumped from `2024-06-01` to `2026-04-01`. When the lib version changes, grep the howto (`agent-base-type.mdx`) for the old literal — the "Applying `@azureBaseType` directly" example hard-codes it.
- `agent.tsp` renamed the `ConversationItem` model to `InputItem`; `ConversationProperties` gained a required `input: InputItem`; message `content` changed from `string` to `Record<unknown>` and `role`/`content` visibility tightened to read-only in `InputItem`/`ResponseItem`. These are internal to the model docs; the howto doesn't enumerate them so no howto edit was needed.
...
- `Relationship<Properties extends RelationshipProperties = RelationshipProperties>` is an `ExtensionResource` template that applies `@azureBaseType(#{ baseType: BaseType.Relationship, version: "2026-04-01" })` automatically. `BaseType.Relationship` is a new member of the `BaseType` enum in `base-types.tsp`.
- `RelationshipProperties<ProvisioningState extends string = ResourceProvisioningState>` supplies the required schema: read-only `baseTypes: BaseTypeInfo[]`, `sourceId`, `sourceTenant`, `targetId`, `targetTenant`, and read-only `provisioningState?`.
- Operations use the `Extension.*` templates against a scope, including the new `Extension.ListByTarget<Scope, Resource>` (in `lib/extension/operations.tsp`) for listing relationships pointing at a target.
- New linting rule `use-relationship-required-properties` (registered in `src/linter.ts`): a resource marked with the Relationship base type must be an extension resource carrying the required relationship schema. The canonical rule doc is `packages/typespec-azure-resource-manager/src/rules/use-relationship-required-properties.md` (tracked); the website copy under `libraries/azure-resource-manager/rules/` is **gitignored** (`.gitignore` ignores `website/src/content/docs/docs/libraries/*/rules/`) and generated at website build time, so never hand-edit it — fix the `src/rules/*.md` file instead.
- Canonical sample: `packages/samples/specs/resource-manager/resource-types/relationship/main.tsp` (uses `#suppress "...basetypes-experimental"`, custom `KeyName`/`SegmentName`/`NamePattern`, and `Extension.ListByTarget`).
