...
Rule documentation is authored in `packages/typespec-azure-resource-manager/src/rules/*.md`.
`pnpm regen-docs` materializes those pages under
`website/src/content/docs/docs/libraries/azure-resource-manager/rules/`; that generated directory is
gitignored. The generated `reference/linter.md` links to those pages. A source rule doc's frontmatter
`title` must match the registered rule name, and its "Full name" block must use the
`@azure-tools/typespec-azure-resource-manager/` package namespace.

## Reference Documentation Generation

- `pnpm regen-docs` from `packages/typespec-azure-resource-manager/` generates the tracked files in
  `reference/` (data-types.md, decorators.md, interfaces.md, index.mdx, linter.md) and materializes
  the gitignored rule pages under `rules/`.
...
`BillingDataProperty` is not an envelope property. Spread it into the resource-specific properties
model. Its `BillingData` schema was added in ARM common types v6.
...
- `@azureBaseType(#{ baseType, version })` (from `base-types.tsp`,
  `Azure.ResourceManager.BaseTypes`) marks a resource model as conforming to a base type.
  `BaseTypeInfo` has `baseType` and `version` fields. Applying it in a non-`Azure.ResourceManager`
  namespace emits the `basetypes-experimental` warning, so user specs must
  `#suppress "@azure-tools/typespec-azure-resource-manager/basetypes-experimental" "..."`.
- `Agent<Properties>` is a `TrackedResource` template that applies the Agent contract version
  `2026-04-01` automatically. Child templates: `AgentConversation<Properties, AgentResource>` and
  `AgentResponse<Properties, AgentResource>` (both `ProxyResource`,
...
- Child property bases `ConversationProperties` and `ResponseProperties` each require
  `input: InputItem` on create. `InputItem` selects its variant with `ItemType`; message content and
  function arguments use `Record<unknown>`. Response mix-ins are `PreviousResponseProperty`,
  `ResponseOutputProperty`, and `ResponseInstructionsProperty`.
...
- `Relationship<Properties extends RelationshipProperties = RelationshipProperties>` is an
  `ExtensionResource` template that applies the Relationship contract version `2026-04-01`.
- `RelationshipProperties<ProvisioningState extends string = ResourceProvisioningState>` supplies
  required `baseTypes`, `sourceId`, `sourceTenant`, `targetId`, and `targetTenant` properties, plus
  optional read-only `provisioningState`. `baseTypes` is also read-only.
- Model operations with the `Extension` templates. The canonical sample uses a reusable
  `Scope extends Foundations.SimpleResource` interface and instantiates it with
  `Extension.ScopeParameter`.
- `use-relationship-required-properties` reports Relationship resources that are not extension
  resources or omit required Relationship properties.
- Canonical sample:
